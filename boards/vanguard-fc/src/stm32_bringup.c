/****************************************************************************
 * boards/arm/stm32h7/vanguard-fc/src/stm32_bringup.c
 *
...
int stm32_bringup(void)
{
  ...
  /* Register and Mount ROMFS */
  ret = stm32_romfs_initialize();
  if (ret < 0) {
    syslog(LOG_ERR, "ERROR: Failed to register ROMFS: %d\n", ret);
  } else {
    syslog(LOG_INFO, "ROMFS mounted at /etc.\n");
  }
   ...
   * and board_composite_connect() in stm32_composite.c.
   * This is triggered automatically by the NSH USB console startup when
   * CONFIG_CDCACM_CONSOLE=y and CONFIG_USBDEV_COMPOSITE=y.
   */

#if defined(CONFIG_NET_CDCECM) && !defined(CONFIG_USBDEV_COMPOSITE)
  /* Standalone CDC/ECM only -- used when composite is disabled */
